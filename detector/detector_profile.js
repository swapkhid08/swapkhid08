$(document).ready(function () {
    ...
    $.ajax({
        url: '../../../models/detector/get_detector_profile_filter.hh',
        type: 'POST',
        success: function (res) {
    ...
    $('#getProfile').click(function () {
        ...
        $.ajax({
            url: '../../../models/detector/get_detector_profile.hh',
            ...
            success: function (result) {
                ...
                for (var i = 0; i < profiledata.length; i++) {
                    ...
                    if (i == 0) {
                        data += '{"time":"' + today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate() + ' ' + profiledata[i].times + '","Profile":"' + profiledata[i].Profile + '","UpperBound":"' + profiledata[i].UpperBound + '","LowerBound":"' + profiledata[i].LowerBound + '"}';
                    } else {
                        data += ',{"time":"' + today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate() + ' ' + profiledata[i].times + '","Profile":"' + profiledata[i].Profile + '","UpperBound":"' + profiledata[i].UpperBound + '","LowerBound":"' + profiledata[i].LowerBound + '"}';
                    }
    ...
    $('#compare').click(function () {
        ...
        var type = $('input[name=graph]:checked').val();
        $.ajax({
            url: '../../../models/detector/get_detector_profile_compare.hh',
            ...
            success: function (result) {
                ...
                var profileid1 = scn.trim()+'_'+dayType;
                var profileid2 = scn.trim()+'_'+comparedayType;
                //console.log(res.data, res.peakintervals)
                //console.log(JSON.parse(res.data))
                ...
                for (var i = 0; i < profiledata.length; i++) {
                    if (i == 0) {
                        ...
                        if(profileid1 != profileid2 && algorithmType != comparealgorithmType && algorithmType != '' && comparealgorithmType != ''){
                            if(profileid1 == profiledata[i].ProfileID){
                                data += ',"'+dayTypename+' - '+algorithmTypeName+'":"' + profiledata[i].Profile + '","'+comparedayTypename+' - '+comparealgorithmTypeName+'":""';
                                bardata += ',"'+dayTypename+' - '+algorithmTypeName+'":"' + profiledata[i].Profile + '","'+comparedayTypename+' - '+comparealgorithmTypeName+'":"0"';
                            }else{
                                data += ',"'+dayTypename+' - '+algorithmTypeName+'":"","'+comparedayTypename+' - '+comparealgorithmTypeName+'":"' + profiledata[i].Profile + '"';
                                bardata += ',"'+dayTypename+' - '+algorithmTypeName+'":"0","'+comparedayTypename+' - '+comparealgorithmTypeName+'":"' + profiledata[i].Profile + '"';
                            }
                        }else{
                            if(profileid1 != profileid2){                            
                                if(profileid1 == profiledata[i].ProfileID){
                                    data += ',"'+dayTypename+'":"' + profiledata[i].Profile + '","'+comparedayTypename+'":""';
                                    bardata += ',"'+dayTypename+'":"' + profiledata[i].Profile + '","'+comparedayTypename+'":"0"';
                                }else{
                                    data += ',"'+dayTypename+'":"","'+comparedayTypename+'":"' + profiledata[i].Profile + '"';
                                    bardata += ',"'+dayTypename+'":"0","'+comparedayTypename+'":"' + profiledata[i].Profile + '"';
                                }
                            }
                            if(algorithmType != comparealgorithmType && algorithmType != '' && comparealgorithmType != ''){
                                if(algorithmType == profiledata[i].AlgorithmType){
                                    data += ',"'+algorithmTypeName+'":"' + profiledata[i].Profile + '","'+comparealgorithmTypeName+'":""';
                                    bardata += ',"'+algorithmTypeName+'":"' + profiledata[i].Profile + '","'+comparealgorithmTypeName+'":"0"';                                                                
                                }else if(comparealgorithmType == profiledata[i].AlgorithmType){
                                    data += ',"'+algorithmTypeName+'":"","'+comparealgorithmTypeName+'":"' + profiledata[i].Profile + '"';
                                    bardata += ',"'+algorithmTypeName+'":"0","'+comparealgorithmTypeName+'":"' + profiledata[i].Profile + '"';
                                }
                            }
                        }
...
function createBarGraph (cdata) {
    ...
    function parsed (d) {
        ...
        // adding calculated data to each count in preparation for stacking
        var y0 = 0; // keeps track of where the "previous" value "ended"
        value.counts = ["UpperBound", "Profile", "LowerBound"].map(function (name) {
            return {
                name: name,
                y0: 0,
                // add this count on to the previous "end" to create a range, and update the "previous end" for the next iteration
