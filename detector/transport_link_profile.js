...
$(document).ready(function(){
    $.ajax({
        url:'../../utils/get_signal_detector_static.php',
        type: 'POST',
    ...
    $('#getProfile').click(function(){
        ...
        $("#graph").html(template);
        var column = sanitize($('#column').val());
        var scn = sanitize($('#scn').val());
        var startTime = $('#startTime').val();
        var endTime = $('#endTime').val();
        var dayType = $('#dayType').val();
        var deviceType = $('#deviceType').val();
        var name = sanitize($('#scn option:selected').attr('name'))

        var type = sanitize($('input[name=graph]:checked').val())

        $.ajax({
            url: '../../utils/get_detector_profile.php',
            data: {column:column,scn:scn,startTime:startTime,endTime:endTime,dayType:dayType},
            type : 'POST',
...
function createBarGraph(cdata){
    ...
    var colour = d3.scale.ordinal()
                        .range(["#98abc5", "#8a89a6", "#7b6888", "red", "black", "green", "#ff8c00"]);
