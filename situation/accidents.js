...
$(document).ready(function () {
    ...
    var mainType = getUrlParameter("type");
    // console.log(mainType);

    $('#main_type').val(mainType).attr("selected", "selected");
    ...
    map.on('click', function (e) {
        ...
        if (route_marker == 'not-created') {
            if (map_point_count == 0) {
                ...
                $("#add_northing").val(e.latlng.lat);
                $("#add_easting").val(e.latlng.lng);
                map.addLayer(markers);
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $.alert({
                    ...
                    Ok: function () {
                        window.location.href = "accidents.html";
                    }
                });
                window.location.href = "accidents.html";
            }
    ...
    //Date picker
    $(".datepicker").datetimepicker({ format: 'd-m-Y H:i:s' });
    ...
    $('#add_acc_btn').click(function () {
        ...
        if(!sdesc){
            $.alert({
                type: 'red',
                title: '<h3 class="text-danger fw-bold mb-0">Message</h3>',
                content: '<div class="fw-bold">Please insert Short Description</div>'
            });
            return;
        }
        ...
        if (ActualStart && !ActualEnd) {
            $.alert({
                type: 'red',
                title: '<h3 class="text-danger fw-bold mb-0">Message</h3>',
                content: '<div class="fw-bold">Actual End Date is Required</div>'
            });
            return;
        }
        ...
        if ((situationType == "accident" || situationType == "incident" || situationType == "crime") && accDT == "") {
            $.alert({
                type: 'red',
                title: '<h3 class="text-danger fw-bold mb-0">Message</h3>',
                content: '<div class="fw-bold">Situation Time cannot be empty</div>'
            });
            return;
        }
        ...
        if ((mid == "event" || mid == "roadwork") && (datetime > PlannedStart)) {
            $.alert({
                type: 'red',
                title: '<h3 class="text-danger fw-bold mb-0">Message</h3>',
                content: '<div class="fw-bold">Planned Start time cannot be a Past time for this Situation. Please enter correct time.</div>'
            });
            return;
        }
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            onChangeSituationType(mainType);
        }
    ...
    function checklist_popup(scn,tid,aid){
        ...
        function add_chk_list(list){
            for(i=list.length-1; i>=0;i--){
                ...
                append += '<td><button class="btn btn-primary" style="background:#253654" value="'+list[i]+'"id="chk_list_solve'+i+'">View</button>'
                append += '</tr>'
                $('#chk_list').append(append);
                $('#chk_list_solve'+i).click(function(e){
                    e.preventDefault();
                    var value = sanitize($(this).val());
                    $.ajax({
                        url:'../../../models/situation/get_accident_types.hh',
                        method:'POST',
                        success:function(res){
                            ...
                            for(var i=0;i<res.length;i++){
                                if (res[i].name == value) {
                                    if (value=="Nearest Camera" && res[i].action=="cctvWindow.html") {
                                        var distance_vms=0,distance_cctv=0,vms_ip="",cctv_ip="",nvr_ip="",channel="",vms_scn="",cctv_scn="";
                                        nor=$('#add_northing')[0].innerHTML;
                                        eas=$('#add_easting')[0].innerHTML;
                                        $.ajax({
                                            url: '../../utils/cctv_devices.hh',
                                            success: function(result) {
                                                var devices_set = jQuery.parseJSON(result);
                                                for(i=devices_set.length-1; i>=0;i--){
                                                    lat = devices_set[i].Northing
                                    ...
                                    else if (value=="Nearest VMS" && res[i].action=="vms_display_msg.html"){
                                        var distance_vms=0,distance_cctv=0,vms_ip="",cctv_ip="",nvr_ip="",channel="",vms_scn="",cctv_scn="";
                                        nor=$('#add_northing')[0].innerHTML;
                                        eas=$('#add_easting')[0].innerHTML;
                                        $.ajax({
                                            url: '../../utils/vms/vms_devices.php',
                                            success: function(result) {
                                                var devices_set = jQuery.parseJSON(result);
                                                for(i=devices_set.length-1; i>=0;i--){
                                                    lat = devices_set[i].Northing;
