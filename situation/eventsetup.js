...
$(document).ready(function () {
	...
	get_dt = function (pageload='') {
		...
		$('#dataTable').DataTable({
			...
			"order": [[1, "desc"]],
			"ajax": {
				"url": "../../../models/situation/getdt_events.hh",
				"type": "POST",
				"data": function (d) {
					d.from = from;
					d.to = to;
				...
				'dataSrc': function (res) {
					...
					for (var i = 0; i < check.length; i++) {
						return_data.push({
							...
							"createmodel": `<span class="createmodel" scn="${check[i].SystemCodeNumber}" type="accident">Create</span> / <a href="./view_situation_model.html?scn=${check[i].SystemCodeNumber}&type=event" target="_blank">View</a> / <span class="downloadmodel" scn="${check[i].SystemCodeNumber}" type="accident">Download</span>`
						})
					}
			...
			"fnDrawCallback": function (oSettings) {
				...
				$('.viewdetails').click(function () {
					...
					//window.open('view_details.html?module=event&id=' + $('.a_scn', ele).html(), "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=0,left=0,width=" + $(window).width() + ",height=" + $(window).height());
					window.location.href='view_details.html?module=event&id='+$('.a_scn', ele).html();
				})
				...
				$('.createmodel').click(function () {
					...
					var scn = $(this).attr('scn'),
						type = $(this).attr('type')
					var url = `${document.location.protocol}//${GLOBAL.SUMO.IP}:${GLOBAL.SUMO.PORT}/run_model/incident/${scn}/type/6/create`;
					console.log(url)
