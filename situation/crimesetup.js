...
$(document).ready(function () {
	...
	get_dt = function (pageload='') {
		...
		$('#dataTable').DataTable({
			...
			"scrollY": "500px",
			"scrollCollapse": true,
			"columnDefs": [{ orderable: false, targets: [1, 2, 5, 13] }],
			"order": [[0, "desc"]],
			// "columnDefs": [{ orderable: false, targets: [1, 2, 5, 13,34] }],
			// "order": [[0, "desc"]],
			"ajax": {
				"url": "../../../models/situation/getdt_crimes.hh",
				"type": "POST",
				"data": function (d) {
					d.from = from;
					d.to = to;
				...
				'dataSrc': function (res) {
					...
					for (var i = 0; i < check.length; i++) {
						return_data.push({
							...
							"NorthingEnd": check[i].NorthingEnd,
							"EastingEnd": check[i].EastingEnd,
			...
			"fnDrawCallback": function (oSettings) {
				...
				$('.viewdetails').click(function () {
					...
					//window.open('view_details.html?module=crime&id=' + $('.a_scn', ele).html(), "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=0,left=0,width=" + $(window).width() + ",height=" + $(window).height());
					window.location.href='view_details.html?module=crime&id=' + $('.a_scn', ele).html();
					return;
					$('#view_modal').modal()
					...
					$('.v_comptime').html($('.a_comptime',ele).html())
					$('.v_remtime').html($('.a_remtime',ele).html())
					$('.v_status').html(($('.a_status',ele).html() == "--Select--" ? "" : $('.a_status',ele).html()))
					$('.v_dreason').html($('.a_dreason',ele).html())
					$('.v_other').html($('.a_other',ele).html())
					$('.v_q1').html(($('.a_q1',ele).html() == "1" ? "Yes" : "No"))
					$('.v_q2').html(($('.a_q2',ele).html() == "1" ? "Yes" : "No"))
					$('.v_q3').html(($('.a_q3',ele).html() == "1" ? "Yes" : "No"))
					$('.v_remark').html($('.a_remark',ele).html())
					$('.v_adue').html($('.a_adue',ele).html())
					$('.v_apaid').html($('.a_apaid',ele).html())
					$('.v_aunpaid').html($('.a_aunpaid',ele).html())
					$('.v_damageremark').html($('.a_damageremark',ele).html())
					$('.v_imagepath').html('')
		...
		for (var i = 0; i < images.length; i++) {
			$('.up_imagepath').append('<a href="' + images[i] + '" class="imagelink" target="_blank" title="Click to open this Image in a new tab"><img src="' + images[i] + '"/></a>')
				...
				$('.downloadmodels').click(function () {
					...
					//var url = 'http://10.105.104.103/download_situation_model.php?scn=' + scn
					var url = document.location.host + '/models/situation/download_situation_model.hh?scn=' + scn;
