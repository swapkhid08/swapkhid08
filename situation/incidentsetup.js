...
$(document).ready(function () {
	...
	active_mode = function () {
		...
		$("#situationIncident").addClass("nav-active")
	...
	get_dt = function (pageload='') {
		...
		$('#dataTable').DataTable({
			...
			"order": [[2, "desc"]],
			"ajax": {
				"url": "../../../models/situation/getdt_incidents.hh",
				"type": "POST",
				"data": function (d) {
					d.from = from;
					d.to = to;
				...
				'dataSrc': function (res) {
					...
					for (var i = 0; i < check.length; i++) {
						return_data.push({
							...
							"createmodel": `<span class="createmodel" scn="${check[i].SystemCodeNumber}" type="accident">Create</span> / <a href="./view_situation_model.html?scn=${check[i].SystemCodeNumber}&type=incident" target="_blank">View</a> / <span class="downloadmodel" scn="${check[i].SystemCodeNumber}" type="accident">Download</span>`
						})
					}
					return return_data;
				}
			...
			"fnDrawCallback": function (oSettings) {
				...
				$('.viewdetails').click(function () {
					...
					//window.open('view_details.html?module=incident&id=' + $('.a_scn', ele).html(), "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=0,left=0,width=" + $(window).width() + ",height=" + $(window).height());
					window.location.href='view_details.html?module=incident&id=' + $('.a_scn', ele).html();
					return;
				})
				...
				$('.createmodel').click(function () {
					...
					var scn = $(this).attr('scn'),
						type = $(this).attr('type')
					var url = `${document.location.protocol}//${GLOBAL.SUMO.IP}:${GLOBAL.SUMO.PORT}/run_model/incident/${scn}/type/7/create`;
					console.log(url)
	...
	$('#add_acc_btn').click(function () {
		...
		$.ajax({
			...
			type: 'POST',
			data: { SystemCodeNumber: "inc", ShortDescription: sdesc, LongDescription: ldesc, Northing: nor, Easting: eas, TypeID: tid, AccidentTime: accDT, CreatedBy: uname, EndDate: edate, Severity: sev, ReportedBy: rep, ModifiedBy: mod, isedit: 0, MainID: type, Action: action, ComplianceTime: ComplianceTime, RemedialTime: RemedialTime, Status: Status, DetailedReason: DetailedReason, OtherInformation: OtherInformation, Question1: Question1, Question2: Question2, Question3: Question3, Remarks: Remarks, AmountDue: AmountDue, AmountPaid: AmountPaid, AmountUnpaid: AmountUnpaid, DamageRemarks: DamageRemarks, CreationDate: datetime },
			success: function (res) {
				$.ajax({
	...
	// 			method: 'POST',
	// 			data: { SystemCodeNumber: delRecord, type: "incident" },
	// 			success: function (res) {
	// 				if (res)
	// 					location.reload();
				...
				success: function (res) {
					...
					if (res.length == 2) {
						...
						$.confirm({
							...
							buttons: {
								Ok: function () {
									window.location.href='incident_setup.html';
								}
							}
						});
					}
