...
$(document).ready(function () {
	...
	$('#add_acc_btn').click(function () {
		...
		DetailedReason = sanitize($('.detailed_reason').val()) == 'undefined' ? '' : ''
		OtherInformation = sanitize($('.other_information').val()) == 'undefined' ? '' : ''
		Question1 = sanitize($('input[name=q1]:checked').val()) == ''? 0 : 1;
		Question2 = sanitize($('input[name=q2]:checked').val()) == ''? 0 : 1;
		Question3 = sanitize($('input[name=q3]:checked').val()) == ''? 0 : 1;
		Remarks = sanitize($('.remarks').val()) == 'undefined' ? '' : ''
		AmountDue = sanitize($('.amount_due').val()) == '' ? 0 : sanitize($('.amount_due').val())== 'undefined' ? '' : ''
		AmountPaid = sanitize($('.amount_paid').val()) == '' ? 0 : sanitize($('.amount_paid').val())== 'undefined' ? '' : ''
		AmountUnpaid = sanitize($('.amount_unpaid').val()) == '' ? 0 : sanitize($('.amount_unpaid').val())== 'undefined' ? '' : ''
		DamageRemarks = sanitize($('.damageremarks').val())== 'undefined' ? '' : ''

		human_impact = sanitize($('.human_impact').val()) == 'undefined' ? '' : ''
		human_factors = sanitize($('.human_factors').val()) == 'undefined' ? '' : ''
		injury_nature = sanitize($('.injury_nature').val()) == 'undefined' ? '' : ''
		property_damage = sanitize($('.property_damage').val()) == 'undefined' ? '' : ''
		socio_economic = sanitize($('.socio_economic').val()) == 'undefined' ? '' : ''
		causes = sanitize($('.causes').val()) == 'undefined' ? '' : ''
		...
		// var datetimen = getFormattedDate(datetime, 'd-m-y H:m:s', 'Y-m-d H:m:s');
		...
		if (edate != "" && Date.parse(accDT) > Date.parse(edate)) {
			$('.loader').hide();
			$.alert({
				type: 'red',
				title: '<h3 class="text-danger fw-bold mb-0">Message</h3>',
